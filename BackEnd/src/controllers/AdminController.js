...
exports.updateUserNoPermission = (req,res) => {
    ...
    connection.query(`SELECT * FROM User WHERE idUser = ?`, [idUser] ,(err, result) => {
        ...
        return res.status(200).json({
            message: 'Sucessoa ao exibir o usuario desejado.',
            success: true,
            data: result
        })
    })
...
// exports.WhoisAdminOrSuperAdmin = (req,res) => {
//     connection.query(`SELECT * FROM User WHERE status_permission = 'SuperAdmin'`, (err, result) => {
//         if(err){
//             return res.status(500).json({
//                 message: "Erro ao se conectar com o servidor.",
//                 success: false,
