...
import { Calendar, Clock, MapPin, Users, ArrowLeft, BookOpen } from 'lucide-react-native'
  ...
  if (loading) {
    return (
      <View style={styles.center}>
        ...
        <Text style={{ marginTop: 8, color: 'white' }}>Carregando {TypeGroup}…</Text>
      </View>
    )
  }
    ...
    return (
      <View style={styles.container}>
        ...
        <View style={styles.center}>
          ...
          <Text style={styles.emptyText}>Sem grupos para {TypeGroup} no momento.</Text>
          <Text style={styles.emptySubtext}>Volte em breve para novas turmas.</Text>
        </View>
      </View>
    )
  }
      ...
      <ScrollView style={styles.scrollView}>
        ...
        {group.map((item) => (
          <View key={item.idStudyGroup} style={styles.groupCard}>
            <ImageBackground
              source={item.ImageUrl ? { uri: item.ImageUrl } : require('../../../../../assets/images/Jesus-Cristo.png')}
              style={styles.cardImage}
              imageStyle={styles.cardImageStyle}
            >
              <View style={styles.imageOverlay}>
                <Text style={styles.groupName}>{item.NameStudy}</Text>
                <Text style={styles.groupType}>{item.TypeGroup}</Text>
              </View>
            </ImageBackground>
            ...
            <View style={styles.cardContent}>
              ...
              <View style={styles.infoRow}>
                <View style={styles.infoItem}>
                  ...
                  <Text style={styles.infoText}>{item.DayOfWeek || "A definir"}</Text>
                </View>
                
                <View style={styles.infoItem}>
                  <Clock size={18} color="#4A90E2" />
                  <Text style={styles.infoText}>{formateTimeForDatTime(item.StartTime) || "Horário a combinar"}</Text>
                </View>
              </View>
              ...
              <View style={styles.infoRow}>
                <View style={styles.infoItem}>
                  ...
                  <Text style={styles.infoText}>{item.Location || "Local a definir"}</Text>
                </View>
