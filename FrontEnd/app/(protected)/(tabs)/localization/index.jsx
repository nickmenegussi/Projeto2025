...
export default function HomeLocalization() {
  ...
  const centrosExemplo = [
    {
      ...
      description: "Sociedade Espírita Gabriel Dellane, localizada em Esteio-RS, dedicada ao estudo e prática da doutrina espírita com trabalhos de passes, atendimento fraterno e evangelização. Fundada em 1985, nossa casa tem como missão divulgar o espiritismo e oferecer apoio espiritual à comunidade.",
      rating: 4.7,
      totalRatings: 156,
      image: "https://images.unsplash.com/photo-1600607687939-ce8a6c25118c?w=800",
      reviews: [
        { author: "Maria Silva", rating: 5, comment: "Ambiente acolhedor e estudos muito enriquecedores. Os voluntários são muito atenciosos." },
        { author: "João Santos", rating: 4, comment: "Ótimo atendimento, voltarei mais vezes. Só achei o estacionamento um pouco complicado." },
        { author: "Ana Pereira", rating: 5, comment: "Local de muita paz e aprendizado. As palestras são transformadoras." }
      ],
      photos: [
    ...
    },
    {
      ...
      description: "Centro espírita nomeado em homenagem a Gabriel Delanne, dedicado ao estudo e divulgação da doutrina espírita.",
      rating: 4.8,
      totalRatings: 89,
      image: "https://images.unsplash.com/photo-1513475382585-d06e58bcb0e0?w=400",
      reviews: [
        { author: "Ana Costa", rating: 5, comment: "Palestras excelentes e equipe muito prestativa." },
        { author: "Carlos Oliveira", rating: 5, comment: "Local perfeito para estudo e reflexão." }
      ],
      photos: [
        "https://images.unsplash.com/photo-1513475382585-d06e58bcb0e0?w=400",
  ...
  const getLocation = async () => {
    try {
      ...
      if (status !== 'granted') {
        Alert.alert('Permissão negada', 'Permita o acesso à localização para encontrar centros próximos');
      ...
      setTimeout(() => {
        const centrosOrdenados = [...centrosExemplo].sort((a, b) => a.distance - b.distance);
  ...
  const filterCentros = (filter) => {
    ...
    if (filter === 'near') {
      ...
      filtered = filtered.filter(centro => centro.rating >= 4).sort((a, b) => b.rating - a.rating);
    } else if (filter !== 'all') {
      filtered = filtered.filter(centro => centro.activities.includes(filter));
    }
  ...
  const openGoogleMaps = (centro) => {
    const url = `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(centro.address)}`;
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        <View>
          <Text style={styles.headerTitle}>Centros Espíritas Próximos</Text>
        </View>
        <TouchableOpacity onPress={getLocation} style={styles.locationButton}>
          <Crosshair size={24} color="#4A90E2" />
        </TouchableOpacity>
      </View>
      ...
      {showFilters && (
        <View style={styles.filterOptions}>
          ...
          <View style={styles.filterRow}>
            ...
            >
              <Text style={[styles.filterChipText, selectedFilter === 'all' && styles.selectedChipText]}>Todos</Text>
            </TouchableOpacity>
            <TouchableOpacity 
              style={[styles.filterChip, selectedFilter === 'near' && styles.selectedChip]}
              onPress={() => filterCentros('near')}
            >
              <Text style={[styles.filterChipText, selectedFilter === 'near' && styles.selectedChipText]}>Próximos</Text>
            </TouchableOpacity>
            <TouchableOpacity 
              style={[styles.filterChip, selectedFilter === 'rating' && styles.selectedChip]}
              onPress={() => filterCentros('rating')}
            >
              <Text style={[styles.filterChipText, selectedFilter === 'rating' && styles.selectedChipText]}>Melhores avaliações</Text>
            </TouchableOpacity>
          </View>
        </View>
      )}
      ...
      <ScrollView style={styles.listContainer} showsVerticalScrollIndicator={false}>
        ...
        {filteredCentros.length === 0 ? (
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptySubtext}>Tente ajustar os filtros ou busca</Text>
          </View>
        ) : (
          filteredCentros.map(centro => (
            <View key={centro.id} style={styles.centroCard}>
              <Image source={{ uri: centro.image }} style={styles.centroImage} />
              <View style={styles.centroContent}>
                <View style={styles.centroHeader}>
                  <Text style={styles.centroName} numberOfLines={1}>{centro.name}</Text>
                  <View style={styles.distanceBadge}>
                    <Text style={styles.distanceText}>{centro.distance} km</Text>
                  </View>
                </View>
                ...
                {centro.rating && (
                  <View style={styles.ratingContainer}>
                    ...
                    <Text style={styles.ratingText}>({centro.totalRatings})</Text>
                  </View>
                )}
                
                <View style={styles.addressContainer}>
                  <MapPin size={14} color="#666" />
                  <Text style={styles.address} numberOfLines={2}>{centro.address}</Text>
                </View>
                
                <View style={styles.scheduleContainer}>
                  <Clock size={14} color="#666" />
                  <Text style={styles.schedule} numberOfLines={1}>{centro.schedule}</Text>
                </View>
                ...
                <View style={styles.activities}>
                  ...
                  {centro.activities.length > 3 && (
                    <View style={styles.moreActivities}>
                      <Text style={styles.moreActivitiesText}>+{centro.activities.length - 3}</Text>
                    </View>
                  )}
                </View>
