...
const SearchBook = () => {
  ...
  const handleSearch = () => {
    ...
    const chunked = chunkArray(results, 8);
    setSearchResults(chunked);
    setShowResults(true);

    const existingIndex = searchHistory.findIndex(
      (item) => item.text === searchText.trim()
    );
    ...
    AsyncStorage.setItem(
      "bookSearchHistory",
      JSON.stringify(updatedHistory.slice(0, 20))
    );
    setSearchText("");
  };
          ...
          <View style={styles.resultsContainer}>
            <View style={styles.resultsHeader}>
              <Text style={styles.resultsTitle}>
                {searchResults.flat().length} resultado
                {searchResults.flat().length !== 1 ? "s" : ""}
              </Text>
              <TouchableOpacity
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                <FlatList
                  data={item}
                  keyExtractor={(book) => book.idLibrary.toString()}
                  renderItem={({ item: book }) => (
                    <CardCustom
                      data={[book]}
                      aboutBookLoan={[book.bookCategory.includes("empréstimo")]}
                      aboutBookReserves={[
                        book.bookCategory.includes("reserva"),
                      ]}
                    />
                  )}
                  horizontal
                  ...
                  <View style={styles.historyItem}>
                    ...
                    <TouchableOpacity
                      ...
                      onPress={() => {
                        ...
                        setSearchResults(chunkArray(results, 8));
                        setShowResults(true);
                      }}
                    >
...
const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: "#003B73" },
  loadingContainer: {
    flex: 1,
  ...
  contentContainer: { flex: 1, paddingTop: 8 },
  resultsContainer: { flex: 1, paddingHorizontal: 12 },
  resultsHeader: {
    flexDirection: "row",
  ...
  resultsTitle: { fontSize: 16, fontWeight: "600", color: "#FFFF" },
  backButton: {
  ...
  sectionTitle: { fontSize: 18, fontWeight: "700", color: "#FFFF" },
  clearText: { color: "#60A3D9", fontSize: 14, fontWeight: "500" },
  historyItem: {
    backgroundColor: "#FFFFFF",
    borderRadius: 12,
    marginBottom: 20,
  ...
  historyList: { paddingTop: 8, paddingBottom: 20 },
  emptyText: {
    fontSize: 16,
    color: "#64748B",
    marginBottom: 8,
