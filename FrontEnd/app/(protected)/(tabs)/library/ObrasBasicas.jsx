...
export default function ObrasBasicas() {
  const { booksBasicas, loading } = useBooks();
  ...
  const chunkedBooksLoans = chunkArray(
    booksBasicas.filter((book) => book.bookCategory === "emprestimo"),
    8
  );
  const chunkedBooksReserve = chunkArray(
    booksBasicas.filter((book) => book.bookCategory === "reserva"),
    8
  );
  ...
  const data = [
    {
      id: 1,
      type: "Obras Básicas para Empréstimo",
      data: chunkedBooksLoans || [],
    },
    {
      id: 2,
      type: "Obras Básicas para Reserva",
    ...
    <FlatList
      ...
      renderItem={({ item }) => (
        <View style={{ marginBottom: 20, gap: 20 }}>
          ...
          {item.data.map((books) =>
            item.type === "Obras Básicas para Empréstimo" ? (
              <CardCustom
      ...
      ListHeaderComponent={() => (
        <View style={styles.HeaderComponent}>
          ...
          <Text style={styles.header}>Obras Básicas</Text>
        </View>
      )}
