...
export default function UpdateLecturesModal({
  titleModal,
  titleButton,
  iconButton,
  otherStyle
  ...
  function formatTimeForSQL(date) {
    if (!date) return null;
    return new Date(date).toISOString().split("T")[1].slice(0, 8); // Retorna 'HH:MM:SS'
  }
  ...
  async function CreateLecture(event) {
    ...
    try {
      const response = await api.post('/lectures/lectures/create', {
        nameLecture: nameLecture,
        dateLecture: formatDateForSQL(dateLecture),
        timeLecture: formatTimeForSQL(timeLecture),
        description: description,
        link_url: link_url,
        video_url: video_url
      }, {
        headers: {
          Authorization: `Bearer ${token}`, // Aqui está a chave de autorização
        },
      })
      ...
      if (error.response && error.response.data.message === "Sessão expirada, por favor, faça login novamente.") {
              ...
              <form className="p-4 md:p-5">
                <div className="grid gap-4 mb-4 grid-cols-2">
                  ...
                  <div className="col-span-2 sm:col-span-2">
                    ...
                    <input
                      ...
                      onChange={(e) => setLectures({...lectures, video_url: e.target.value})}
                      name="nameLecture"
                      className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                      placeholder="Digite o nome da palestra"
                      required=""
                ...
                <div className="col-span-2 sm:col-span-2">
                  ...
                  <textarea
                    ...
                    onChange={(e) => setLectures({...lectures, description: e.target.value})}
                    rows="2"
